...
class RocketFlightModule : public PositionModule
{
  protected:
    ...
    // meshtastic_Position_AltSource currentAltitudeSource =
    ...
    uint32_t maximumTimeThreshold = Default::getConfiguredOrDefaultMs(config.position.position_broadcast_secs, 60) * 1000;
  ...
  protected:
    ...
    // Draw aligned text
    static void drawText(OLEDDisplay *display, int16_t x, int16_t y, OLEDDISPLAY_TEXT_ALIGNMENT textAlignment,
                         const String &text);
